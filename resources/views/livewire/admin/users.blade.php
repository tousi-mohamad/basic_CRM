            ...
            @foreach($users as $user)
                <tr class="bg-white border-b" :class="darkMode == 0 ? 'dark:bg-gray-900 dark:border-gray-700' : 'bg-amber-50'">
                    <th
                        ...
                        <input id="input_{{$user->id}}" x-bind:type="inputType" class="names_input hidden text-black border-b-cyan-500 border-r" value="{{$user->name}}"
                    ...
                    <td class="py-4 px-6 flex">
                        <svg wire:click="delete({{$user->id}})" id="trash_{{$user->id}}" @click="handleClick" data-timer="5"
